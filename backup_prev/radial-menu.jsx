/* RadialKindMenu — pie/wheel menu for changing a waypoint's kind directly
   on the map. Opens around the clicked node, closes on outside-click / ESC.
   Position auto-clamped inside the canvas viewport. */

function RadialKindMenu({ open, cx, cy, currentKind, kinds, onPick, onClose, kindColors }) {
  ...
  const entries = Object.entries(kinds).filter(([k]) => k !== "custom" && k !== "origin");
  // 8 kinds → distribute around a full circle starting at top
  const N = entries.length;
  const R = 102;   // radius of the segment centers
  const iconR = 26; // radius of the icon disc
  const startAngle = -Math.PI / 2; // top-centered on first item
  ...
  return (
    ...
    >
      ...
      >
        {/* orbit ring */}
        <circle r={R} fill="none" stroke="var(--brass)" strokeOpacity="0.25" strokeWidth="1" strokeDasharray="2 4" />
        <circle r={R - 6} fill="none" stroke="var(--line)" strokeOpacity="0.12" strokeWidth="1" />
        <circle r={R + 6} fill="none" stroke="var(--line)" strokeOpacity="0.10" strokeWidth="1" />
        {/* radial guides */}
        {entries.map((_, i) => {
          ...
          return (
            <line
              ...
              x2={Math.cos(a) * (R + 30)}
              y2={Math.sin(a) * (R + 30)}
              stroke="var(--line)" strokeOpacity="0.08" strokeWidth="0.8"
            />
          );
        })}
      </svg>
      ...
      {/* center hub — indicates selected + closes on click */}
